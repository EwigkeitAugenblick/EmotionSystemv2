...
def related_sort_pic(data, pic):
    score=[]
    original_picture = requests.get(pic, stream=True)
    img1 = Image.open(BytesIO(original_picture.content))
    hash1 = imagehash.average_hash(img1)

    for url in data['封面图片'].values:
        picture = requests.get(url, stream=True)
        img2 = Image.open(BytesIO(picture.content))
...
def video_select(data,title,tag,intro,pic):
    ...
    data_video = pd.concat([data_title, data_tag, data_intro,data_pic])
    return data_video

def data_crawler(aid_list):

    data=pd.DataFrame(columns=['urls','标题','标签','简介','封面图片'])
    ...
    for i in range(0,len(aid_list)):
        ...
        data.loc[i] = [aid_list[i], title, tag, intro, pic]
    return data
...
def keyword_bv2(keyword,BV):
    ...
    res = json.loads(res.text)['data']['result']
